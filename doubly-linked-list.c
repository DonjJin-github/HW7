...
int main()
{
	...
	do{
		...
		case 'z': case 'Z': //command가 z or Z일때
			initialize(&headnode); //initialize함수를 호출하여 head노드 초기화
			break;
		...
		case 'n': case 'N': //command가 n or N일때
			printf("Your Key = ");
			scanf("%d", &key); //사용자에게 key를 입력받는다
			insertLast(headnode, key); //insertLast를 호출하여 headnode 마지막노드에 key삽입
			break;
		...
		case 'f': case 'F': //command가 f or F일때
			printf("Your Key = ");
			scanf("%d", &key); //사용자에게 key를 입력받는다
			insertFirst(headnode, key); //inserFirst를 호출하여 headnode 첫번째노드에 key삽입
			break;
...
int insertLast(headNode* h, int key) {
	...
	{
		h->first = node;//h의 첫번쨰 노드에 node 저장
		return 0; //함수 종료
	}
